                                ...
                                for(MarkingView token : arcView.getWeight())
                                {
                                    if(token.getToken().getID().equals(_id))
                                    {
                                        try
                                        {
                                            ...
                                            _forwardsIncidenceMatrix.set(placeNo, transitionNo, token.getCurrentMarking());//arcView.getWeightOfTokenClass(_id));
                            ...
                            if(pn instanceof TransitionView)
                            {
                                ...
                                for(MarkingView token : arcView.getWeight())
                                {
                                    if(token.getToken().getID().equals(_id))
                                    {
                                        ...
                                        {
                                        	if(isTransitionInfiniteServer){
                                        		_backwardsIncidenceMatrix.set(placeNo, transitionNo, token.getCurrentMarking()*enablingDegree);
                                        	}else{
                                        		_backwardsIncidenceMatrix.set(placeNo, transitionNo, token.getCurrentMarking());//arcView.getWeightOfTokenClass(_id));
                                        	}
